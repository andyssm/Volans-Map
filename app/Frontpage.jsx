...
export default function App() {
  ...
  useEffect(() => {
    const accSub = Accelerometer.addListener(({ x, y, z }) => {
      setAccData({ x, y, z });
      // ⚠️ Do NOT detect steps here; only show acceleration
    });
  ...
  const handleStep = () => {
    if (!recordedHeading.current) return;
    const { latitude, longitude } = vincentyDirect(
      position.latitude,
      position.longitude,
      recordedHeading.current,
      STEP_LENGTH
    );
  ...
  const togglePause = () => {
    setPaused((prev) => {
      const next = !prev;
      if (next) {
        recordedHeading.current = heading;
      }
      return next;
    });
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.displacementBar}>
        <Text style={styles.displacementText}>
          🚶 Displacement: {(stepCount * STEP_LENGTH).toFixed(2)} meters
        </Text>
      </View>
      ...
      <TouchableOpacity
        ...
        disabled={!paused}
      >
        <Text style={styles.resetText}>👣 Step</Text>
      </TouchableOpacity>
    </View>
  );
}
